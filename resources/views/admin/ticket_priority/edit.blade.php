...
@section('dash_page_title')
    {{ __('messages.edit_priority_ticket') }}
@endsection
@section('breadcrumb')
    {{--  {{ Breadcrumbs::render('') }}--}}
@endsection
...
@section('admin_main')
    ...
    <div class="container-fluid">
        ...
        <div class="row d-flex justify-content-start my-4 bg-white">
            <div class="col-lg-4 col-md-4 col  my-5  border-bottom title-add-to-stock">
                <div class="alert my-4">
                    <h3> {{ __('messages.edit_priority_ticket') }}</h3>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('admin.priority.ticket.update') }}" method="post">
            @csrf
            <div class="row product-stock-list mt-5 py-5 bg-white">

                <input type="hidden" name="ticket" value="{{ $priority->id }}">
                <div class="col-sm-6">
                    <div class="mt-3">
                ...
                <div class="col-sm-6">
                    <div class="mt-3">
                        ...
                        <select name="status" class="form-select" id="status">
                            <option value="0" {{ $priority->status == 0 ? 'selected' : '' }} >{{ __('messages.deactivate') }}</option>
                            <option value="1" {{ $priority->status == 1 ? 'selected' : '' }}>{{ __('messages.active') }}</option>
                        </select>
                    </div>
                    @error('status')
                    <div class="alert alert-danger mt-3">
