    ...
    <div class="container-fluid">
        ...
        <div class="row d-flex justify-content-start my-4 bg-white">
            <div class="col-lg-4 col-md-4 col  my-5  border-bottom title-add-to-stock">
                <div class="alert my-4">
                    <h3>{{ __('messages.newest_banner') }} / {{ __('messages.edit_banner') }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="row my-4 bg-white">
            <div class="col-lg-4 col-md-4 col my-2">
                <a href="{{ route('admin.newest.product.index') }}" class="btn btn-sm btn-primary">{{ __('messages.list_banners') }}</a>
            </div>
        </div>


        <form action="{{ route('admin.newest.product.update') }}" method="post" enctype="multipart/form-data">
            @csrf
                            ...
                            <div class="mt-3">
                                ...
                                <select class="form-select" name="title" aria-label="Default select example">
                                    ...
                                    @foreach($newestProducts as $product)
                                        <option value="{{ $product->id }}" {{ $banner->slug == $product->slug ? 'selected' : '' }}>{{ $product->title_persian }} </option>
                                    @endforeach
                                </select>
                            </div>
                            @error('title')
                        ...
                        <div class="col-12">
                            <div class="mt-3">
                                ...
                                <select class="form-control" id="status" name="status">
                                    <option value="1" {{ $banner->status == 1 ? 'selected' : '' }} >{{ __('messages.active') }}</option>
                                    <option value="0" {{ $banner->status == 0 ? 'selected' : '' }} >{{ __('messages.deactivate') }}</option>
                                </select>
                            </div>
                            @error('status')
                            <div class="alert alert-danger mt-3">
                ...
                <div class="col">
                    ...
                    <div class="row mt-4 d-flex flex-column justify-content-center align-content-center">
                        <div class="col-lg-5 d-flex justify-content-center align-content-center top-banner-section">
                            <img src="{{ $banner->image_path && file_exists(public_path() .$banner->image_path) ?
                                        $banner->image_path : asset('admin_assets/images/no-image-icon-23494.png') }}"
                                 id="image_view"
                        ...
                        <div class="col-lg-5">
                            ...
                            <input type="file"
                                   class="form-control" accept="image/png, image/jpeg , image/jpg ,image/gif" id="image_select"
                                   name="image_path">
                            @error('image_path')
                            <div class="alert alert-danger mt-3">
                                {{ $message }}
